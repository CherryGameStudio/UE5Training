...
#include "GASTraining/GameplayAbilitySystem/CharacterAttributeSet.h"
...
void ATP_ThirdPersonCharacter::BeginPlay()
{
	...
	if (HasAuthority())
	{
		UE_LOG(LogTemp, Warning, TEXT("HasAuthority BeginPlay"));
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("Local BeginPlay"));
	}

	if (!HasAuthority())
	{
		//设置Owner Actor & Avatar Actor
...
UCharacterAttributeSet* ATP_ThirdPersonCharacter::GetAttributeSet() const
{
	return AttributeSet;
}

void ATP_ThirdPersonCharacter::InitializeAbilitySystem()
{
	//Ability
	{
		for (auto Ability : GameplayAbilities)
		{
			if (Ability)
			{
				...
				{
					// 不要添加重复技能
					UE_LOG(LogTemp, Warning, TEXT("不要添加重复技能 %s"), *Ability.GetDefaultObject()->GetName());
				}
				else
				{
